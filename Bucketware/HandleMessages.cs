//Oringial code from PlayingoDeerux i have just added more things and edited - Fyrax
using ENet.Managed;
using System;
...
namespace GrowbrewProxy
{
    public class HandleMessages
    {
        ...
        {                      
            switch (vList.FunctionName)
            {
                case "OnConsoleMessage":
                    {
                        ...
                        if ((m.Contains("lagged out,") || m.Contains("experiencing high load")) && !m.Contains("<") && !m.Contains("["))
                        {
                            ...
                            packetSender.SendData(variantPacket2.GetBytes(), Proxy.proxyPeer);
                        }
                ...
                case "OnRequestWorldSelectMenu":
                    {
                        if (Proxy.globalUserData.autoEnterWorld != "")
                        {
                            packetSender.SendPacket(3, "action|join_request\nname|" + Proxy.globalUserData.autoEnterWorld, Proxy.realPeer);
                        }
                        break;
                    }
                    ...
                    try
                    {
                        ...
                        foreach (string line in lines)
                        {
                            if (line.Contains("+"))
                            {
                                ...
                                packetSender.SendPacket(2, resultingPacket, Proxy.realPeer);
                            }
                        }
                        return -1;
                    }
                ...
                case "OnSendToServer":
                    {
                        ...
                        packetSender.SendData(variantPacket.GetBytes(), Proxy.proxyPeer);


                        Proxy.globalUserData.Growtopia_IP = token < 0 ? Proxy.globalUserData.Growtopia_Master_IP : ip;
                        Proxy.globalUserData.Growtopia_Port = token < 0 ? Proxy.globalUserData.Growtopia_Master_Port : port;
                        Proxy.globalUserData.isSwitchingServer = true;
                        Proxy.globalUserData.token = token;
                        Proxy.globalUserData.lmode = 1;
                        Proxy.globalUserData.userID = userID;
                        Proxy.globalUserData.doorid = doorid;

                        packetSender.SendPacket(3, "action|quit", Proxy.realPeer);
                        Proxy.realPeer.Disconnect(0);
                ...
                case "OnSpawn":
                    {
                        ...
                        {
                            if (Proxy.globalUserData.cheat_autoworldban_mod) banEveryoneInWorld();
                        }
                        ...
                        if (localplayer)
                        {
                            ...
                            if (Proxy.globalUserData.unlimitedZoom)
                            {
        ...
        private void SwitchServers(ref ENetPeer peer, string ip, int port, int lmode = 0, int userid = 0, int token = 0)
        {
            ...
            Proxy.ConnectToServer(ref peer, Proxy.globalUserData);
        }
        ...
        public string HandlePacketFromClient(ref ENetPeer peer, ENetPacket packet) // Why string? Oh yeah, it's the best thing to also return a string response for anything you want!
        {
            ...
            if (Proxy.realPeer.IsNull) return "";
            if (Proxy.realPeer.State != ENetPeerState.Connected) return "";
                ...
                case NetTypes.NetMessages.GENERIC_TEXT:
                    ...
                    if (str.StartsWith("action|"))
                    {
                        ...
                        if (actionExecuted.StartsWith("enter_game"))
                        {
                            ...
                            if (Proxy.globalUserData.blockEnterGame) return "Blocked enter_game packet!";
                            enteredGame = true;
                        }
                        ...
                        foreach (string line in lines)
                        {
                            ...
                            switch (lineToken[0])
                            {
                                case "tankIDName":
                                    ...
                                    break;
                                case "tankIDPass":
                                    Proxy.globalUserData.tankIDPass = lineToken[1];
                                    break;
                                case "requestedName":
                                    Proxy.globalUserData.requestedName = lineToken[1];
                                    break;
                                case "token":
                                    Proxy.globalUserData.token = int.Parse(lineToken[1]);
                                    break;
                                case "user":
                                    Proxy.globalUserData.userID = int.Parse(lineToken[1]);
                                    break;
                                case "lmode":
                                    Proxy.globalUserData.lmode = int.Parse(lineToken[1]);
                                    break;

                            }
                        }
                        Proxy.globalUserData.tankIDName = tankIDName;

                        bool hasAcc = false;

                        packetSender.SendPacket((int)NetTypes.NetMessages.GENERIC_TEXT, Proxy.CreateLogonPacket(), Proxy.realPeer);
                ...
                case NetTypes.NetMessages.GAME_MESSAGE:
                    ...
                    if (str2.StartsWith("action|"))
                    {
                        ...
                        if (actionExecuted.StartsWith("quit") && !actionExecuted.StartsWith("quit_to_exit"))
                        {
                            ...
                            respondToBotPeers = true;
                            Proxy.globalUserData.token = -1;
                            Proxy.globalUserData.Growtopia_IP = Proxy.globalUserData.Growtopia_Master_IP;
                            Proxy.globalUserData.Growtopia_Port = Proxy.globalUserData.Growtopia_Master_Port;

                            if (Proxy.realPeer != null)
                            {
                                if (!Proxy.realPeer.IsNull)
                                    if (Proxy.realPeer.State != ENetPeerState.Disconnected) Proxy.realPeer.Disconnect(0);
                            }
                            if (Proxy.proxyPeer != null)
                            {
                                if (!Proxy.proxyPeer.IsNull)
                                    if (Proxy.proxyPeer.State == ENetPeerState.Connected) Proxy.proxyPeer.Disconnect(0);
                            }
                        }
                ...
                case NetTypes.NetMessages.GAME_PACKET:
                    {
                        ...
                        switch ((NetTypes.PacketTypes)(byte)p.PacketType)
                        {
                            ...
                            case NetTypes.PacketTypes.PLAYER_LOGIC_UPDATE:
                                ...
                                Proxy.globalUserData.isFacingSwapped = IsBitSet(p.CharacterState, 4);
                    ...
                    "\nPunchX|PunchY: " + p.PunchX + "|" + p.PunchY);*/
                                    ...
                                    Proxy.globalUserData.lastWrenchX = (short)p.PunchX;
                                    Proxy.globalUserData.lastWrenchY = (short)p.PunchY;
                                }
                                else if (p.MainValue == 18 && Proxy.globalUserData.redDamageToBlock)
                                {
                                    // playingo
                                    p.SecondaryPadding = -1;
                                    p.ExtDataMask |= 1 << 27; // 28
                                    p.Padding = 1;
                                    packetSender.SendPacketRaw(4, p.PackForSendingRaw(), Proxy.realPeer);
                                    return "";
                                }
                                break;
                            case NetTypes.PacketTypes.ITEM_ACTIVATE_OBJ: // just incase, to keep better track of items incase something goes wrong
                                ...
                                if (Proxy.globalUserData.blockCollecting) return "";
                                break;
                            default:
            ...
            packetSender.SendData(data, Proxy.realPeer);
        ...
        private void SpoofedPingReply(TankPacket tPacket)
        {
            ...
            p.SecondaryNetID = (int)Proxy.HashBytes(BitConverter.GetBytes(tPacket.MainValue)); // HashString of it

            // rest is 0 by default to not get detected by ac.
            packetSender.SendPacketRaw((int)NetTypes.NetMessages.GAME_PACKET, p.PackForSendingRaw(), Proxy.realPeer);
        }

        public string HandlePacketFromServer(ref ENetPeer peer, ENetPacket packet)
        {

            if (Proxy.proxyPeer.IsNull) return "HandlePacketFromServer() -> Proxy peer is null!";
            if (Proxy.proxyPeer.State != ENetPeerState.Connected) return $"HandlePacketFromServer() -> proxyPeer is not connected: state = {Proxy.proxyPeer.State}";
            ...
            switch (msgType)
            {
                case NetTypes.NetMessages.SERVER_HELLO:
                    {
                        Proxy.LogText += ("[" + DateTime.UtcNow + "] (SERVER): Initial logon accepted." + "\n");
                        Proxy.UserData ud;

                        if (peer.TryGetUserData(out ud))
                            packetSender.SendPacket(2, Proxy.CreateLogonPacket(ud.tankIDName, ud.tankIDPass, ud.userID, ud.token, ud.doorid), peer);
                ...
                case NetTypes.NetMessages.GAME_PACKET:
                    ...
                    switch (packetType)
                    {
                        ...
                        case NetTypes.PacketTypes.INVENTORY_STATE:
                            {
                                if (!Proxy.globalUserData.dontSerializeInventory) 
                        ...
                        case NetTypes.PacketTypes.CALL_FUNCTION:
                            ...
                            Proxy.LogText += ("[" + DateTime.UtcNow + "] (SERVER): A function call was requested, see log infos below:\nFunction Name: " + VarListFetched.FunctionName + " parameters: " + argText + " \n");
                            ...
                            if (VarListFetched.FunctionName == "OnSetPos" && Proxy.globalUserData.ignoreonsetpos && netID == worldMap.netID) return "Ignored position set by server, may corrupt doors but is used so it wont set back. (CAN BE BUGGY WITH SLOW CONNECTIONS)";
                        ...
                        case NetTypes.PacketTypes.LOAD_MAP:
                            if (Proxy.LogText.Length >= 32678) Proxy.LogText = string.Empty;
                            ...
                            Proxy.realPeer.Timeout(1000, 2800, 3400);
                        ...
                        case NetTypes.PacketTypes.MODIFY_ITEM_OBJ:
                            {
                                ...
                                if (p.NetID == -1)
                                {
                                    ...
                                    if (Proxy.globalUserData.cheat_magplant)
                                    {
                                        ...
                                        packetSender.SendPacketRaw((int)NetTypes.NetMessages.GAME_PACKET, p2.PackForSendingRaw(), Proxy.realPeer);
            ...
            packetSender.SendData(data, Proxy.proxyPeer);
