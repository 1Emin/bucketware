...
namespace GrowbrewProxy
{
    ...
    public class World
    {
        ...
        public int readPos = 0; // good to keep track of world serialization pos :)
        ...
        public int GetUserNetIDByName(string name)
        {
            foreach (Player pl in players)
            {
                if (pl.name == name) return pl.netID;
        ...
        public Point GetLocalPlayerPos()
        {
            Player playerObject = player;
            int playerNetID = playerObject.netID;
            foreach (Player pl in players)
            {
                if (1 == netID) return new Point(pl.X, pl.Y);
            }
        ...
        public int GetPositionX(int netID)
        {
            foreach (Player pl in players)
            {
                if (pl.netID == netID) return pl.X;
            }
            return -1;
        }
        public int GetPositionY(int netID)
        {
            foreach (Player pl in players)
            {
                if (pl.netID == netID) return pl.Y;
            }
            return -1;
        }
        ...
        private int Tile_Serialize(byte[] dataPassed, int loc)
        {
            try
            {
                ...
                {
                    Program.LogText += ("[" + DateTime.UtcNow + "] (PROXY): [" + currentWorld + "] readPos bigger than dataPassed.Length error during world serialization!\n");
                    return -1;
                }
        ...
        private int WorldObjects_Serialize(byte[] dataPassed)
        {
            try
            {
                ...
                Program.LogText += ("[" + DateTime.UtcNow + "] (PROXY): Serialized +1 dropped item object. ID: " + dropped.id.ToString() + " X:" + dropped.x.ToString() + " Y: " + dropped.y.ToString() + " UID: " + dropped.uid.ToString() + "\n");
            }
            catch (ArgumentException)
            {
                return -1;
        ...
        public World LoadMap(byte[] packet)
        {
            try
            {
                ...
                Program.LogText += ("[" + DateTime.UtcNow + "] (PROXY): Loading world: " + currentWorld + ", total tile count is: " + tileCount.ToString() + "\n");
                if (!Program.globalUserData.serializeWorldsAdvanced)
                {
                    Program.LogText += ("[" + DateTime.UtcNow + "] (PROXY): NOTE: Advanced world serialization was disabled, not continuing to load tiles.\n");
                    return this;
                }
                ...
                Program.LogText += ("[" + DateTime.UtcNow + "] (PROXY): [" + currentWorld + "]" + " Tiles properly serialized (without any errors): " + tilesProperlySerialized.ToString() + "\n");
                if (readPos >= data.Length)
                {
                    Program.LogText += ("[" + DateTime.UtcNow + "] (PROXY): [" + currentWorld + "] readPos error during world serialization!\n");
                    return this;
                }
                droppedItems.Clear();
                ...
                dropped_ITEMUID = count;
                total_bytes_serialized = (ulong)readPos;
                Program.LogText += ("[" + DateTime.UtcNow + "] (PROXY): Succeeded loading world '" + currentWorld + "'! Total bytes serialized: " + total_bytes_serialized.ToString() + ",  dropped item uid count: " + count + "\n");
            }
            catch
            {
                return this;
